...
import {
  ...
  getReportsData,
} from "@/features/transactions/reportsActions";
...
export default function DashboardPage() {
  ...
  const month = searchMonth ? Number(searchMonth) : new Date().getMonth();
  const year = searchYear ? Number(searchYear) : new Date().getFullYear();
        ...
        <div className="bg-surface p-5 md:p-6 rounded-2xl border border-border shadow-sm group hover:border-red-500/30 transition-all flex flex-col justify-between">
          <div className="flex justify-between items-start mb-2 text-text-sub font-medium">
            ...
            <div className="size-8 rounded-full bg-red-500/10 flex items-center justify-center transition-colors group-hover:bg-red-500/20">
              <span className="material-symbols-outlined text-[18px] text-red-500/80">
                payments
              </span>
            </div>
          </div>
          <div>
            <AmountDisplay
              ...
              symbolClassName="text-red-500/60"
            />
            <p className="text-[10px] uppercase font-bold tracking-wider text-text-dim mt-3 italic opacity-60">
              Acumulado del mes
            </p>
          </div>
        </div>
        ...
        <div className="bg-surface p-5 md:p-6 rounded-2xl border border-border shadow-sm group hover:border-blue-500/30 transition-all flex flex-col justify-between">
          <div className="flex justify-between items-start mb-2 text-text-sub font-medium">
            ...
            <div className="size-8 rounded-full bg-blue-500/10 flex items-center justify-center transition-colors group-hover:bg-blue-500/20">
              <span className="material-symbols-outlined text-[18px] text-blue-500/80">
          ...
          <div>
            <AmountDisplay
              value={monthlyBudget}
              className="text-3xl font-black text-text-main"
              symbolClassName="text-blue-500/50"
            />
            <p className="text-[10px] uppercase font-bold tracking-wider text-text-dim mt-3 italic opacity-60">
              Límite definido
            </p>
        ...
        <div className="bg-surface p-6 rounded-2xl border border-border shadow-sm group hover:border-primary/30 transition-all flex flex-col justify-between">
          <div>
            <div className="flex justify-between items-start mb-3">
              <h3 className="text-text-sub text-sm font-bold uppercase tracking-widest text-[10px]">
                Saldo Disponible
              </h3>
              <span
                className={`text-[10px] font-black uppercase tracking-widest px-2 py-1 rounded-lg transition-colors ${
                  usedPercentage > 90
            ...
            <AmountDisplay
              ...
              symbolClassName={
                remainingBudget >= 0 ? "text-primary/60" : "text-red-500/40"
              }
            />
          </div>
          ...
          mobileContent={
            recentTransactions.length === 0 ? (
              ...
              recentTransactions.map((trx: any) => (
                <div
                  key={trx.id}
                  className="p-4 flex flex-col gap-3 group active:bg-surface-hover/50 transition-colors"
                >
